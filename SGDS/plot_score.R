...
score <- read.table("/home/yolanda/tblab/yolanda/GLOWgenes/panelAPP/GLOWgenes_RDs/score/score_median.tsv", header = TRUE, sep = ",")
#score <- read.table("/home/yolanda/tblab/yolanda/GLOWgenes/panelAPP/analysis/score/score_median_with_NAs.tsv", header = TRUE, sep = ",")
...
basic + coord_cartesian(clip = "off") +
  geom_label_repel(
    data = top_high_score,
    aes(label = Symbols, fill = n_panelapp),
    size = 4, max.overlaps = 25,
    box.padding = unit(0.35, "lines"),
    point.padding = unit(0.3, "lines"),
...
################################33
 ...
 #View(subset(glowmatrix_w_score[which(glowmatrix_w_score$Retinal_disorders_GA>0),], (Retinal_disorders_GA < 250 & Score < 0.1))[,c("Symbols","Retinal_disorders_GA", "Score")])
#View(subset(glowmatrix_w_score[which(glowmatrix_w_score$Retinal_disorders_GA>0),], (Retinal_disorders_GA < 250 & Score > 0.75))[,c("Symbols","Retinal_disorders_GA", "Score")])

#View(glowmatrix_w_score[,c("Symbols","Retinal_disorders_GA", "Score")])
#View(glowmatrix_w_score %>% dplyr::filter(Retinal_disorders_GA == 0) %>% dplyr::select("Symbols","Retinal_disorders_GA", "Score", "median_ranking"))
specific_genes_DHR <- subset(glowmatrix_w_score[which(glowmatrix_w_score$Retinal_disorders_GA>0),], (Retinal_disorders_GA < 250 & Score > 0.75))[c("Symbols", "Retinal_disorders_GA")]
score %>% dplyr::filter(Symbols %in% specific_genes_DHR$Symbols)

non_specific_genes_DHR <- subset(glowmatrix_w_score[which(glowmatrix_w_score$Retinal_disorders_GA>0),], (Retinal_disorders_GA < 250 & Score < 0.3))[c("Symbols", "Retinal_disorders_GA")]
score %>% dplyr::filter(Symbols %in% specific_genes_DHR$Symbols)

top_DHR <- subset(glowmatrix_w_score[which(glowmatrix_w_score$Retinal_disorders_GA>0),], (Retinal_disorders_GA < 100))[c("Symbols", "Retinal_disorders_GA")]
top_DHR_score <- score %>% dplyr::filter(Symbols %in% top_DHR$Symbols)

top_DHR <- merge(top_DHR, top_DHR_score, by = "Symbols")
...
top_DHR %>%
  arrange((Retinal_disorders_GA))

top_DHR <- subset(glowmatrix_w_score[which(glowmatrix_w_score$Retinal_disorders_GA>0),], (Retinal_disorders_GA < 250))[c("Symbols", "Retinal_disorders_GA")]
top_DHR_score <- score %>% dplyr::filter(Symbols %in% top_DHR$Symbols)
