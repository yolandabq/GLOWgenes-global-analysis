setwd("/home/graciela/Desktop/excels_metadata/")
setwd("/home/graciela/tblab/graciela/glowgenes/")
setwd("/Users/gracielauria/Desktop/glowgenes/")
library(utils)
library(R.utils)
library(vcfR)
library(stats)
...
############################# 1) INVESTIGUE LA BD DE UNIPROT PERO AL FINAL NO LA USE ########################
##download uniprot
...
library(UniProt.ws)
# Create a UniProt query object
up_query <- UniProt.ws::(query = "taxonomy:9606", fields = c("entry name", "genes(PREFERRED)", "families(Pfam)"))
# Query UniProt for human proteins with Pfam annotations




# Retrieve protein data
up_data <- UniProt.ws::select(up_query, limit = NA)# Extract gene names and Pfam families/categories
gene_pfam <- up_data[, c("Entry", "Gene", "Pfam")]

# Clean up the data (remove NAs, duplicates, etc. if necessary)
gene_pfam <- na.omit(gene_pfam)  # Remove rows with missing values
gene_pfam <- unique(gene_pfam)   # Remove duplicate rows


############################# 2) INVESTIGUE LOS GO TERMS PERO AL FINAL NO LOS USE ########################
######################## gene ontology terms #######################################
...
##genes asociados a cada goterm (los IDs)
mapping_raw <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol", "go_id"), mart = ensembl) #457934 entradas (asociacion gen a un GO term)
head(mapping) #457934
mapping<-mapping_raw %>%
  dplyr::filter(go_id != "") #397567
...
GO<- as.data.frame(GO.db::GOTERM) #134297
##all GO terms:Ontology:  molecular function: 31781, biological process:   95801 and cellular component: 6714
table(GO$Ontology)
GO<-unique(GO)
GO_MF_raw<-GO %>%
  dplyr::filter(GO$Ontology=="MF") #31781 molecular functions
GO_MF<-GO_MF_raw[,c(1,2,3)]
GO_MF<-unique(GO_MF) #11236 molecular functions
b<-as.data.frame(b) 



###READ MATRIX DE GENES POR PANELES
glowmatrix <- read.delim("glowmatrix.tsv", header = T, row.names = 1, stringsAsFactors = F, quote = "", check.names=F)
#extraer lista de genes: hgnc
all_genes<-rownames(glowmatrix)
length(unique(all_genes)) #verificar que son unicos: 24757

my_mapping<-mapping %>% 
  dplyr::filter(mapping$hgnc_symbol %in% all_genes) #380954

##merge el GO dataframe de todos los GO terms con definitions con el my_mapping
merged_df <- merge(my_mapping, GO, by = "go_id", all.x = TRUE)
table(merged_df$hgnc_symbol)

############################# 3) INVESTIGUE LA BD DE KEGG PERO AL FINAL NO LA USE ########################
################## KEGG REST SERVER API: https://www.kegg.jp/kegg/rest/keggapi.html

setwd("/home/graciela/Downloads/")
BiocManager::install("KEGGREST")
BiocManager::install("keggorthology")
library(KEGGREST)
library(keggorthology)
c<-keggorthology::keggDF2graph("/home/graciela/Downloads/ko00001.keg")
a<- read.delim("glowmatrix.tsv", header = T, row.names = 1, stringsAsFactors = F, quote = "", check.names=F)

brite_hierarchies <- KEGGREST::keggList("brite")
brite_hierarchies <- KEGGREST::keggList("ko")
gg<-as.data.frame(brite_hierarchies)

pathway_info <- keggGet("K04397")
gene<-pathway_info[[1]][["SYMBOL"]]
gene_entries <- pathway_info$GENES

pathway_info <- keggGet("BR:ko00001")
...
############## obtain ko00001.keg with all the hierarchy, parse it and obtain the dataframe ######

#1) download keg k00001.keg file
#https://www.kegg.jp/kegg-bin/get_htext?ko00001.keg

#2) parse keg: uso un codigo de un paquete de R: https://www.bioconductor.org/packages/release/bioc/html/keggorthology.html
#es la funcion parse_keg.R 
a<-parseKeg("/home/graciela/Downloads/ko00001.keg") #le meto mi archivo: /home/graciela/Downloads/ko00001.keg
#la movida es que parsea solo hasta el nivel 3 (A,B,C) no parsea el nivel 4 que son las familias de genes
#2) run dfWdepth.R: convertir mi lista a a un dataframe con otra funcion de esta genete: 
...
setwd("/home/graciela/tblab/graciela/glowgenes/panther/")
...
gur_parsed_df <- read.table("panther/panther_gene_protein_class_gur.txt", header = TRUE, sep = "\t")
...
norm_glow_matrix<- read.delim("glowmatrix_normalized.tsv", header = T, row.names = 1, stringsAsFactors = F, quote = "", check.names=F)
...
################## ESTO AL FINAL NO LO USO, DIRECTAMENTE HAGO EL TSNE DF DE ABAJO ##################
#2) pca
pca_result_no_fam <- prcomp(no_fam[, -ncol(no_fam)], scale. = TRUE) #perform pca on all columns except last one
pca_df <- as.data.frame(pca_result_no_fam$x)
pca_df$level1_name <- no_fam$level1_name

# Basic ggplot structure
basic <- ggplot(pca_df, aes(x = PC1, y = PC2, color = level1_name)) +
  geom_point() +
  labs(title = "PCA Plot_NOFAM", x = "PC1", y = "PC2") +
  geom_vline(xintercept = 0, linetype = "dotted") +  # Add a vertical line at x = 0
  geom_hline(yintercept = 0, linetype = "dotted")    # Add a horizontal line at y = 0

# Use facet_wrap correctly
basic + facet_wrap(~ level1_name, nrow = 7)
...
#cambiar escala 
random_colors <- randomcoloR::randomColor(length(unique(no_fam$level1_name)))
basic<-ggplot2::ggplot(data = as.data.frame(pca_result_no_fam$x), aes(x = PC1, y = PC2, color = no_fam$level1_name)) +
  geom_point() +
  labs(title = "PCA Plot_NOFAM", x = "PC1", y = "PC2") + 
...
###cargar matrices de Yoli 30  septiembre 2024
glowmatrix <- read.table("glow_matrix_clustered_no_NA.tsv", header = TRUE, sep = "\t")
glowmatrix<-glowmatrix[,-c(1)]
...
set.seed(42)  # Setting seed for reproducibility
glowmatrix <- na.omit(glowmatrix) #19148 -> quitar NAs de glow, info sin clase
invert_tsne_result <- Rtsne::Rtsne(glowmatrix, dims = 2, perplexity = 30, verbose = TRUE, max_iter = 300)
...
color_panels <- read.table("final_panels_list.txt", header = FALSE, sep = "\t")
...
ggplot2::ggsave(combined_plot,filename = "/home/graciela/Desktop/figura1.png",width = 20, height = 11)
